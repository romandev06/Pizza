...
import { addItem, CartItem, deleteAllItems, deleteCurrentItem, minusItem } from '../../redux/slices/cartSlice'
                ...
                {cartItems.map((item: CartItem, i: number) => 
                    <div key={i} className='pizza-cart__values'>
                        ...
                        <div className='pizza-cart__count'>
                            <button disabled={item.count === 1} className='minus-count__btn' onClick={() => dispatch(minusItem(item))}>
                                <img className={clsx({'img-disabled': item.count === 1})} src="/appImages/minus-count-btn.svg" alt="minus" />
                            </button>
                            <p>{item.count}</p>
                            <button className='plus-count__btn' onClick={() => dispatch(addItem(item))}>
                                <img src="/appImages/plus-count-btn.svg" alt="plus" />
                            </button>
                        </div>
